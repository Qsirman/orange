...
B = bwboundaries(separation_V,'noholes');
for k=1:length(B)
    boundary = B{k};
    if length(boundary(:,1)) < 100
        separation_V(boundary(:,1),boundary(:,2))=0;
    end
end
...
% se=strel('disk',18);
open_V = imdilate(separation_V,se);
