...
kernel = cv2.getStructuringElement(cv2.MORPH_CROSS,(8,18))
...
kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (4, 4))  # 十字形结构
edge_V = cv2.dilate(edge_V, kernel)  # 膨胀
...
#     print('R3',R3)
    ...
    temp__ = np.intersect1d(np.where(points[:,0,0] == A[0]),np.where(points[:,0,1] == A[1]))
    indexA = temp__[0]

    temp__ = np.intersect1d(np.where(points[:,0,0] == B[0]),np.where(points[:,0,1] == B[1]))
    indexB = temp__[0]

    temp__ = np.intersect1d(np.where(points[:,0,0] == C[0]),np.where(points[:,0,1] == C[1]))
    indexC = temp__[0]
    ...
    while T > e:
        ...
        while count < maxIterator:
            ...
            #d=1
            if plus == 1:
                indexB_ = indexB_ + random.randint(-steps,steps)
                indexC_ = indexC_ + random.randint(-steps,steps)
                # B_[0] = B_[0] + d
                # C_[1] = C_[1] + d
            elif plus == 2:
                indexA_ = indexA_ + random.randint(-steps,steps)
                indexC_ = indexC_ + random.randint(-steps,steps)
                # A_[0] = A_[0] + d
                # C_[1] = C_[1] + d
            elif plus == 3:
            ...
            indexA_ = max(0,indexA_)
            indexA_ = min(len(points),indexA_)

            indexB_ = max(0,indexB_)
            indexB_ = min(len(points),indexB_)

            indexC_ = max(0,indexC_)
            indexC_ = min(len(points),indexC_)

            A_ = np.array([points[indexA_,0,0],points[indexA_,0,1]])
            B_ = np.array([points[indexB_,0,0],points[indexB_,0,1]])
            C_ = np.array([points[indexC_,0,0],points[indexC_,0,1]])
            R_ = getM(points,A_,B_,C_)
            ...
            print('T=',T)
            print('P=',P)
            if m > 0.85 and (df < 0 or P > random.random()):
                ff = 1
